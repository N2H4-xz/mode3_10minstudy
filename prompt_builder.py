...
5. 若某个特征描述中提到"混合"或"有时……有时……"，在回答样式要求中必须体现这种双重性。

【重要约束：只输出格式指令，不输出内容倾向】
6. 生成的 prompt 必须只描述说话格式（句式长短、标点习惯、回答结构、语气等），绝不提及话题偏好、专业方向或内容特征。
7. 【参考例句】部分必须在所有例句之前加上这句话：「以下例句仅用于展示说话格式，模仿时只参考其句式结构与语气节奏，不要参考例句中的具体内容。」
8. 不得在 prompt 中写入任何暗示用户话题偏好的内容，即使风格画像中的例句涉及具体话题。
"""
