...
5. 【说话风格总结】和【回答样式要求】必须严格围绕以下六类论文指标组织：词汇复杂度、句法复杂度、形式化指数、情感表达度、可读性指标、人际交往标记。
6. 若某个特征描述中提到"混合"或"有时……有时……"，在回答样式要求中必须体现这种双重性。

【重要约束：只输出格式指令，不输出内容倾向】
7. 生成的 prompt 必须只描述说话格式（词汇复杂度、句法复杂度、正式程度、情感表达、可读性、人际标记等），绝不提及话题偏好、专业方向或内容特征。
8. 不得新增论文六类之外的独立风格分类；如果需要提到标点、句长、语气词等，只能放入对应论文类别中。
...
def _generate_template_version(profile: StyleProfile) -> str:
    ...
    sections.append("以下特征严格按论文 3.2 Stylistic Feature Extraction 的六类指标组织：\n")
    for spec in CHARACTERISTICS_SPEC:
        key = spec["key"]
        c: Characteristic | None = profile.get(key)
